...
def test_voigt_apex():
    gamma = 1.2
    peak = voigt(jnp.array(center), amplitude, center, sigma, gamma)
    assert peak.item() == amplitude


def test_voigt_gaussian_limit():
    gamma = 1e-4
    peak_v = voigt(x, amplitude, center, sigma, gamma)
    peak_g = gaussian(x, amplitude, center, sigma)
    assert bool(jnp.allclose(peak_v, peak_g, atol=1e-3))
